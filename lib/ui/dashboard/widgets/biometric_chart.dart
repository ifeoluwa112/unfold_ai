...
class BiometricChart extends StatelessWidget {
            ...
            LineChartBarData(
              ...
              dotData: FlDotData(
                ...
                getDotPainter: (spot, percent, barData, index) {
                  ...
                  return FlDotCirclePainter(radius: 2, color: _getChartColor());
                },
              ),
            ...
            touchCallback: (event, response) {
              if (response?.lineBarSpots?.isNotEmpty == true) {
                ...
                if (index < processedData.length) {
                  onDateSelected(processedData[index].dateTime);
                }
              }
            },
          ),
  ...
  List<BiometricData> _processData() {
    ...
    if (filteredData.isNotEmpty &&
        (isLargeDataset || filteredData.length > 100)) {
      final targetPoints = selectedRange == DataRange.sevenDays ? 50 : 100;
  ...
  List<HorizontalLine> _getHorizontalLines(
    ...
    // Add mean line for HRV
    final values = data.map((d) => d.hrv).toList();
